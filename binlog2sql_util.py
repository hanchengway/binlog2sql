...
def parse_args():
    ...
    parser.add_argument('--gtid-interval', dest='gtid_interval', type=str,
                        help="GTID Interval. format uuid:1 or uuid:1-2 or uuid:1-2,uuid:3-4;", default='')
...
def concat_sql_from_binlog_event(cursor, binlog_event, row=None, e_start_pos=None, flashback=False, no_pk=False,
    ...
    if isinstance(binlog_event, WriteRowsEvent) or isinstance(binlog_event, UpdateRowsEvent) \
        ...
        sql += ' #start %s end %s time %s gtid %s' % (e_start_pos, binlog_event.packet.log_pos, time, gtid_interval)
